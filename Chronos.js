/*!
 * Chronos.js v1.1.0 — Media-query-free responsive CSS engine
 * Usage:  <script>Chronos.respond('style.css');</script>
 *
 * Changelog:
 ...
 *            <style type="text/chronos"> tags, rem-based accessibility
 *            scaling for fluid(), and the chronosCompiled debug event.
 *
 * Chronos Writes CSS like:
 ...
 *
 * Chronos parses the raw CSS, converts these opt-in helper functions into
...
(function (global) {
  ...
  var VERSION = '1.1.0';
  var STYLE_ID = 'chronos-compiled-styles';
  ...
  var _explainLog = {};      // selector -> generated CSS, for Chronos.explain()
    ...
    rules.forEach(function (rule) {
      ...
      decls.forEach(function (decl) {
        ...
        if (fluidMatch) {
          ...
          // Anything not using a Chronos helper function is left exactly
  ...
  var NAV_STYLE_ID = 'chronos-nav-styles';
  var ICON_FONT_ID = 'chronos-icon-font';
  ...
  function injectNavStyles(breakpoint) {
    ...
    var css = [
      '.chronos-hamburger{display:none;align-items:center;justify-content:center;',
      'width:40px;height:40px;border:none;background:transparent;cursor:pointer;padding:0;z-index:1101;}',
      '.chronos-hamburger i{font-size:22px;color:var(--chronos-icon-color,#141414);transition:transform .2s ease;}',
      '.chronos-hamburger.is-open i{transform:rotate(90deg);}',
      ...
      '.chronos-managed-nav{display:none;}',
      '.chronos-nav-backdrop{display:none;}',
      ...
      '  .chronos-desktop-nav{display:none !important;}',
      '  .chronos-hamburger{display:flex;}',
      ...
      '  .chronos-managed-nav{position:fixed;top:0;right:0;height:100vh;width:min(80vw,340px);',
      ...
      '  .chronos-managed-nav.is-open{transform:translateX(0);}',
      '  .chronos-slide-title{color:#5b6472;font-size:12px;letter-spacing:.12em;',
      '    text-transform:uppercase;margin:0 0 1rem;font-weight:600;}',
      '  .chronos-slide-list{list-style:none;margin:0;padding:0;display:flex;flex-direction:column;}',
      '  .chronos-slide-list li{border-bottom:1px solid rgba(255,255,255,.08);}',
      '  .chronos-slide-link{display:block;padding:1rem 0.25rem;color:#f3f4f6;',
      ...
      '  .chronos-slide-link:active,.chronos-slide-link:hover{color:#7dd3fc;}',
      '  .chronos-managed-nav.is-open .chronos-slide-link{opacity:1;transform:translateX(0);}',
      // staggered entrance
      '  .chronos-managed-nav.is-open .chronos-slide-list li:nth-child(1) .chronos-slide-link{transition-delay:.05s;}',
      '  .chronos-managed-nav.is-open .chronos-slide-list li:nth-child(2) .chronos-slide-link{transition-delay:.1s;}',
      '  .chronos-managed-nav.is-open .chronos-slide-list li:nth-child(3) .chronos-slide-link{transition-delay:.15s;}',
      '  .chronos-managed-nav.is-open .chronos-slide-list li:nth-child(4) .chronos-slide-link{transition-delay:.2s;}',
      '  .chronos-managed-nav.is-open .chronos-slide-list li:nth-child(5) .chronos-slide-link{transition-delay:.25s;}',
      '  .chronos-managed-nav.is-open .chronos-slide-list li:nth-child(n+6) .chronos-slide-link{transition-delay:.3s;}',

      '  .chronos-nav-backdrop{display:block;position:fixed;inset:0;background:rgba(0,0,0,.45);opacity:0;',
      '    pointer-events:none;transition:opacity .25s ease;z-index:1099;}',
      '  .chronos-nav-backdrop.is-open{opacity:1;pointer-events:auto;}',
      '}'
    ].join('\n');
  ...
  function buildHamburger() {
    ...
    btn.className = 'chronos-hamburger';
  ...
  // Builds a fresh, Chronos-styled panel from the original nav's links,
  ...
  function buildSlidePanel(nav) {
    ...
    panel.className = 'chronos-managed-nav';
    panel.setAttribute('aria-hidden', 'true');

    var title = document.createElement('p');
    title.className = 'chronos-slide-title';
    title.textContent = 'Menu';
    panel.appendChild(title);

    var list = document.createElement('ul');
    list.className = 'chronos-slide-list';

    nav.querySelectorAll('a').forEach(function (originalLink) {
      var li = document.createElement('li');
      var link = document.createElement('a');
      link.className = 'chronos-slide-link';
  ...
  function initMobileNav(options) {
    ...
    if (!nav || nav.dataset.chronosNav === 'true') return;
    ...
    nav.classList.add('chronos-desktop-nav');
    nav.dataset.chronosNav = 'true';

    var panel = buildSlidePanel(nav);
    document.body.appendChild(panel);

    var backdrop = document.createElement('div');
    backdrop.className = 'chronos-nav-backdrop';
    document.body.appendChild(backdrop);

    var hamburger = buildHamburger();
    hamburger.style.setProperty('--chronos-icon-color', pickIconColor(header));
  ...
  // ---------------------------------------------------------------------
  // file:// fallback — scan for inline <style type="text/chronos"> tags
  // ---------------------------------------------------------------------
  //
  ...
  // with nothing to compile. As a fallback, Chronos scans the document
  // for <style type="text/chronos"> tags and compiles those instead, so
  // local, no-server testing still works:
  //
  //   <style type="text/chronos">
  //     .hero { font-size: fluid(24px, 48px); }
  //   </style>
  //   <script>Chronos.respond('style.css');</script>

  function scanDocumentForStyles(options) {
    var styleTags = document.querySelectorAll('style[type="text/chronos"]');
    if (!styleTags.length) {
      console.warn('Chronos: no fallback <style type="text/chronos"> tags found either. ' +
        'Serve the page over http(s):// or add an inline text/chronos style tag.');
      return;
    }
  ...
  function respondText(cssText, options) {
    ...
    if (options.debug) {
      console.log('%cChronos.js generated CSS', 'font-weight:bold');
      console.log(output);
    }
    ...
    // exactly what was compiled, without needing Chronos.explain().
    window.dispatchEvent(new CustomEvent('chronosCompiled', {
  ...
  var Chronos = {
    ...
    respond: function (cssPath, options) {
      return fetch(cssPath)
        .then(function (res) {
          if (!res.ok) throw new Error('Chronos: could not load ' + cssPath + ' (' + res.status + ')');
          return res.text();
        })
        ...
        .catch(function (err) {
          console.warn('Chronos: fetch failed for "' + cssPath + '" (likely opened via file:// ' +
            'without a local server). Falling back to inline <style type="text/chronos"> tags…', err);
          scanDocumentForStyles(options);
        });
    },

    // Compile a raw CSS string directly — skips fetch() entirely. Useful
    // for the file:// fallback above, or for feeding Chronos a stylesheet
    ...
    explain: function (selector) {
      ...
      if (!found) {
        console.log('Chronos.explain: no generated CSS found for "' + selector + '". ' +
  ...
  // calls respond() at all but does have inline text/chronos style tags,
  // compile them automatically once the DOM is ready.
  window.addEventListener('DOMContentLoaded', function () {
    if (document.querySelectorAll('style[type="text/chronos"]').length && !document.getElementById(STYLE_ID)) {
